...
git -C ~/hermes-agent apply --check -v ~/hermes-friends-starter/patches/voice-and-desktop-features.patch
```
...
Check the command path in `config.yaml` actually exists — if you cloned the starter somewhere other than `~/hermes-friends-starter`, `tts.providers.jarvis.command` still points at the old path. Use an absolute path. Test it standalone:
